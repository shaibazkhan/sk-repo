...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    if(initial.isEmpty) Stream.empty
      ...
      val more = for {
        path <- initial
        onlyNewNeighbours <- newNeighborsOnly(neighborsWithHistory(path._1, path._2), explored)
      } yield onlyNewNeighbours
   ...
   * position.
   */
    //list.reduceLeft((l,r) => if (r.age < l.age) r else l)
  lazy val solution: List[Move] = {
    val paths = pathsToGoal
    if(paths.isEmpty) List() else paths.head._2.reverse
  }
